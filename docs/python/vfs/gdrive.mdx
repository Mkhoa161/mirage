...
| `getfattr`      | Extended attributes, incl. the Drive id    |
...
A file's or folder's Drive id reads back as the extended attribute
`user.mirage.file_id`, so a folder's id can be passed as a parent
without searching for it:

```bash
parent=$(getfattr -n user.mirage.file_id --only-values /gdrive/Reports)
gws drive files create --json "{\"name\": \"notes\", \"parents\": [\"$parent\"]}"
```

The attribute belongs to Drive, so `setfattr` refuses to change it
with `Operation not permitted`. Attributes under any other name are
stored by Mirage and work on every mount.
