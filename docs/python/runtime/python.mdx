...
The pinned Monty 0.0.21 lacks `dir()`, `json.load()` and `json.dump()`;
these remain absent in 0.0.23. Use `hasattr(value, 'name')` to check a
specific attribute, `json.loads(f.read())` to read JSON, and
