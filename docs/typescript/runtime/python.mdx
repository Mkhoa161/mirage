...
the attributes the shell's `getfattr` and `setfattr` see, including a
backend's read-only facts under `user.mirage.` (a Google Drive file's id
is `user.mirage.file_id`). They need the worker; the fallback below
answers them with `OSError` (`ENOTSUP`).
