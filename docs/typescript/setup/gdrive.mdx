...
A file's or folder's Drive id reads back as the extended attribute `user.mirage.file_id` (`getfattr -n user.mirage.file_id --only-values /drive/Reports`), which is the id `gws` takes for a parent.
