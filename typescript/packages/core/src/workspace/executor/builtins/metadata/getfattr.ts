 ...
 * the path, and the backend's own facts as `user.mirage.<key>` (a Drive
 * file id reads as `user.mirage.file_id`). `-h` reads a link's own
 * attributes. Mirrors Python's `handle_getfattr`.
 */
