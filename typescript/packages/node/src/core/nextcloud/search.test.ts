...
function multistatus(paths: { href: string; directory?: boolean }[]): string {
  const responses = paths
    .map(({ href, directory }) => {
      ...
      const vfsType = directory === true ? '<d:collection/>' : ''
      return `<d:response><d:href>${href}</d:href><d:propstat><d:prop><d:displayname>${name}</d:displayname><d:vfstype>${vfsType}</d:vfstype><d:getcontentlength>42</d:getcontentlength><oc:size>42</oc:size><d:getlastmodified>Sat, 11 Jul 2026 12:00:00 GMT</d:getlastmodified></d:prop><d:status>HTTP/1.1 200 OK</d:status></d:propstat></d:response>`
    })
    .join('')
